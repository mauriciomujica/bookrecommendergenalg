...
def main():
    ...
    sim_users = core.gen_alg.sim_matrix(
    ...
    ratings_sim_score = core.gen_alg.book_similarity(
        ratings_filtered, ratings_filtered_isbn, sim_df
    )
    means = core.gen_alg.mean_users(similar_users, ratings_filtered)
    ...
    pop = core.gen_alg.initialPop(rated_items, books, M, N)

    while currentGen != maxGen:
        df = pd.DataFrame(pop, columns=[f"Book_{i + 1}" for i in range(N)])
        book_vectors = core.gen_alg.get_vectors(df, books)
        correlations = core.gen_alg.correlationCal(book_vectors, N)
        ...
        newpop = core.gen_alg.crossover(bestMem, int(len(df) * R), N)
        df2 = pd.DataFrame(newpop, columns=[f"Book_{i + 1}" for i in range(N)])
        sim_scores = core.gen_alg.similarityCal(book_sim, dic_sim, df2)
        ...
        nextgenpop = core.gen_alg.crossover(bestMem2, int(len(df) * R), N)
    ...
    pr = core.gen_alg.predict(df3, ratings_filtered_isbn, ratings, targetUser, isbn_to_userid, ratings_filtered, sim_df)
